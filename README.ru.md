...
- [dot](./gns/utils/dot.py): Вычисляет умножение `a @ b` для `a` и `b` одного ранга (оба 2-го или оба 3-го ранга).
...
- [load_binary](./gns/utils/load_binary.py): Загружает значение из файла, сериализованного модулем pickle.
...
- [mixed_mode_dot](./gns/utils/mixed_mode_dot.py): Вычисляет эквивалент функции `tf.einsum('ij, bjk->bik', a, b)`.
- [modal_dot](./gns/utils/modal_dot.py): Вычисляет матричное умножение для `a` и `b`.
- [normalized_adjacency](./gns/utils/normalized_adjacency.py): Нормализует заданную матрицу смежности.
...
- [sp_matrices_to_sp_tensors](./gns/utils/sp_matrices_to_sp_tensors.py): Преобразование разреженных матриц Scipy в тензор.
- [sp_matrix_to_sp_tensor](./gns/utils/sp_matrix_to_sp_tensor.py): Преобразует разреженную матрицу Scipy в разреженный тензор.
- [to_disjoint](./gns/utils/to_disjoint.py): Преобразует списки узловых объектов, матриц смежности и граничных объектов в непересекающийся режим.
- [to_tf_signature](./gns/utils/to_tf_signature.py): Преобразует сигнатуру набора данных в сигнатуру тензорного потока.
