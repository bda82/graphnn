...
def scatter_mean(messages, indices, n_nodes):
    """
    Averages messages according to segments defined by `indexes`, with
    support for messages in single/disjoint mode (single/disjoint) (rank 2) and mixed mode (rank 3).
    The output data has the same rank as the input data, with the dimension "nodes", changed to the value of the n_nodes parameter.
